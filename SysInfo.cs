...
if (unscaledDeltaTime < SysInfo.minTime)
{ 
unscaledDeltaTime = SysInfo.minTime;
}
...
if (realtimeSinceStartup - this.sys_lastUpdateTime < SysInfo.updateInterval)
{ 
return;
}
if (this.sys_accumulatedTime < SysInfo.minTime)
{ 
this.sys_accumulatedTime = SysInfo.minTime;
}
...
this.memtotal.text = string.Format("{0:0.0#} MB", SysInfo.ConvertBytesToMegabytes(Profiler.GetTotalReservedMemoryLong()));
this.memalloc.text = string.Format("{0:0.0#} MB", SysInfo.ConvertBytesToMegabytes(Profiler.GetTotalAllocatedMemoryLong()));
if (Debug.isDebugBuild)
{ 
this.memgxf.text = string.Format("{0:0.0#} MB", SysInfo.ConvertBytesToMegabytes(Profiler.GetAllocatedMemoryForGraphicsDriver()));
return;
}
this.memgxf.text = "";
